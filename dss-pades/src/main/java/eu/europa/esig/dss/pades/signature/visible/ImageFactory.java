...
public class ImageFactory {
    ...
    public static Dimension getOptimalSize(SignatureImageParameters imageParameters) throws IOException {
        ...
        if (imageParameters.getImage() != null) {
            InputStream in = new ByteArrayInputStream(imageParameters.getImage());
    ...
    public static InputStream create(final SignatureImageParameters imageParameters) throws IOException {
        ...
        if ((textParamaters != null) && StringUtils.isNotEmpty(textParamaters.getText())) {
            ...
            if (imageParameters.getImage() != null) {
                ...
                switch (textParamaters.getSignerNamePosition()) {
                    case LEFT:
                        buffImg = ImagesMerger.mergeOnRight(ImageIO.read(in), buffImg, textParamaters.getBackgroundColor());
                        break;
                    case RIGHT:
                        buffImg = ImagesMerger.mergeOnRight(buffImg, ImageIO.read(in), textParamaters.getBackgroundColor());
                        break;
                    case TOP:
                        buffImg = ImagesMerger.mergeOnTop(ImageIO.read(in), buffImg, textParamaters.getBackgroundColor());
                        break;
                    case BOTTOM:
                        buffImg = ImagesMerger.mergeOnTop(buffImg, ImageIO.read(in), textParamaters.getBackgroundColor());
                        break;
                    default:
                        break;
                }
            ...
            return new ByteArrayInputStream(imageParameters.getImage());
        }
    }
